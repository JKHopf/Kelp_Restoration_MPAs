function [N, Nbio, Ybio, Rs] = Func_ProjPop_KR4pop_BH(scenario, fishaffect, SpParas,...
...
%     RF = repmat(RF,1,1,size(Rs,2),nsim);
    ...
    if strcmp(fishaffect, 'yes')
        if Rs(t,i)<1 && t>Yr_distR
            Rs(t,i) = 1-SpParas.Sg^(-Abio+log(1-Rs(t,i))/log(SpParas.Sg)); % Rs(t,i) + Rg.*Abio;
        end
        if As(t,i)<1 && t>Yr_distA
            As(t,i) = 1-SpParas.Sg^(-Abio+log(1-As(t,i))/log(SpParas.Sg)); % As(t,i) + Ag.*Abio;
        end
    end
     
    % this only begins to apply the year after the RESTORATION is done
    % Same as no fish-to-hab benefit as restoration is instananeous
%     if strcmp(fishaffect, 'yes')
%         if Rs(t,i)==1 && t>Yr_distR
%             Rs(t,i) = 1-SpParas.Sg^(-Abio+log(1-Rs(t,i))/log(SpParas.Sg)); % Rs(t,i) + Rg.*Abio;
%         end
...
%     end
    ...
    R(i) = (SpParas.BHa.*Rs(t,i))./(1+SpParas.BHa.*Rs(t,i).*S./SpParas.BHb);
    ...
    M(Mmin:Mmax,Mmin:Mmax)= M(Mmin:Mmax,Mmin:Mmax)+...
                            diag(diag(M(Mmin:Mmax,Mmin:Mmax),-1).*As(t,i)-...
    ...
    switch scenario
        case 'fished' 
            ...
            Ybio = Df.*Nbio; 
        case 'reserve'
            Y = [zeros(SpParas.A_max*2,size(N,2));
                 repmat(Df,SpParas.A_max*2,size(N,2))].*N; 
            Ybio = [zeros(SpParas.A_max*2,size(N,2));
                    repmat(Df,SpParas.A_max*2,size(N,2))].*Nbio;
        case 'unfished'
            Y = 0.*N;
            Ybio = 0.*Nbio;
    end
...
%     % Multiply recruits into projection matrix
%     % i.e. fecundity * area * p.c. survival                                    
%                 M(i*SpParas.A_max-SpParas.A_max+1,:,:,:) = repmat(...
%                 M(i*SpParas.A_max-SpParas.A_max+1,...
%                   i*SpParas.A_max-SpParas.A_max+1:i*SpParas.A_max,:,:)...
%                 .*Area_vec(i).*R(i),1,p);
%             
%     % multiply in disturbance effects on adults
%     diagAs(t,i)